...
function makeTraffic(){const colors=[0x9e3434,0x345f8a,0xd0c4a5,0x292d31,0x64845a,0x8c6333];for(let i=0;i<34;i++){const horizontal=i%2===0,p=-105+Math.random()*210;makeCar(horizontal?p:(i%4<2?7:48),horizontal?(i%4<2?7:48):p,colors[i%colors.length],horizontal?0:Math.PI/2,true)}}
...
function updateVehicle(dt){if(!currentVehicle||!locked||paused)return;const car=currentVehicle;const accelerate=keys.KeyW?1:keys.KeyS?-1:0;const boost=keys.ShiftLeft||keys.ShiftRight?1.35:1;const maxSpeed=16*boost;const targetSpeed=accelerate*maxSpeed;car.userData.speed=THREE.MathUtils.lerp(car.userData.speed,targetSpeed,accelerate?0.075:0.045);const steering=(keys.KeyA?-1:0)+(keys.KeyD?1:0);const reverse=car.userData.speed<0?-1:1;if(Math.abs(car.userData.speed)>.4)car.rotation.y+=steering*dt*(1.15+Math.abs(car.userData.speed)/20)*reverse;const heading=new THREE.Vector3(Math.cos(car.rotation.y),0,Math.sin(car.rotation.y));const next=car.position.clone().add(heading.multiplyScalar(car.userData.speed*dt));if(blocked(next.x,next.z,1.9)||Math.abs(next.x)>118||Math.abs(next.z)>116){car.userData.speed*=.35}else car.position.copy(next);speedEl.textContent=`${Math.round(Math.abs(car.userData.speed)*4.9)} KM/H`;car.rotation.z=THREE.MathUtils.lerp(car.rotation.z,-steering*.035,dt*7)}
function updateTraffic(dt){for(const car of cars){if(car===currentVehicle||!car.userData.traffic)continue;const heading=new THREE.Vector3(Math.cos(car.rotation.y),0,Math.sin(car.rotation.y));car.position.add(heading.multiplyScalar(car.userData.speed*dt));if(car.position.x>122||car.position.x<-122)car.position.x=car.position.x>0?-120:120;if(car.position.z>146||car.position.z<-116)car.position.z=car.position.z>0?-114:144}}
function spawnPolice(){const p=currentVehicle?currentVehicle.position:player.position,angle=Math.random()*Math.PI*2,dist=34+Math.random()*18;makeNPC(p.x+Math.cos(angle)*dist,p.z+Math.sin(angle)*dist,true);if(wanted>=2)makeCar(p.x+Math.cos(angle)*dist,p.z+Math.sin(angle)*dist,0x243b5c,angle,false,true)}
